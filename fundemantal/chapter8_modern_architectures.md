...
## 8.1 Introduction to Modern Architectures

Traditional CNNs like ResNet and VGG have been superseded by more efficient architectures that achieve better accuracy with fewer parameters and computations.

## 8.2 EfficientNet: Scaling CNNs

EfficientNet uses a compound scaling method that uniformly scales depth, width, and resolution.

### Key Innovation: Compound Scaling
```
depth = α^φ    (network depth)
width = β^φ     (network width)
...
# Usage
model = EfficientNet(num_classes=10)  # For CIFAR-10
print(f"EfficientNet parameters: {sum(p.numel() for p in model.parameters()):,}")
```

## 8.3 MobileNetV3: Efficient Mobile Networks

MobileNetV3 combines depthwise separable convolutions with neural architecture search.

### Key Features:
- **Hard Swish activation**: More efficient than Swish
- **Squeeze-and-Excitation** blocks
- **Neural Architecture Search** optimized blocks
...
# Usage
model = MobileNetV3(num_classes=10)  # For CIFAR-10
print(f"MobileNetV3 parameters: {sum(p.numel() for p in model.parameters()):,}")
```

## 8.4 Vision Transformers (ViT)

Vision Transformers apply transformer architecture to images by treating image patches as tokens.

### Key Concept: Patch Embedding
```python
...
# Usage (note: ViT typically needs large datasets and pre-training)
model = VisionTransformer(
    image_size=224,  # ViT typically trained on 224x224
    patch_size=16,
...
| Architecture | Parameters | FLOPs | Top-1 Acc | Best For |
|-------------|------------|-------|-----------|----------|
| ResNet-50 | 25.6M | 4.1B | 76.3% | General purpose |
| EfficientNet-B0 | 5.3M | 0.4B | 77.1% | Mobile/Edge |
| MobileNetV3-Large | 5.4M | 0.2B | 75.2% | Mobile devices |
| ViT-Base/16 | 86.6M | 17.6B | 77.9% | Large datasets |

### When to Use Each Architecture:

**EfficientNet**: When you need the best accuracy-efficiency trade-off
**MobileNetV3**: When deploying to mobile/edge devices with limited compute
**Vision Transformer**: When you have large datasets and want state-of-the-art accuracy
**ResNet**: When you need a proven, reliable architecture for most tasks

## 8.6 Code: Training Modern Architectures
...
def train_modern_cnn():
    """Train EfficientNet on CIFAR-10"""

    # Data
    transform = transforms.Compose([
        transforms.Resize(224),  # EfficientNet expects larger images
        transforms.ToTensor(),
...
This chapter introduces you to state-of-the-art CNN architectures that achieve better performance with greater efficiency than traditional networks.</content>
<parameter name="filePath">/Users/chhayhong/Downloads/CNN_Masterclass_Project/fundemantal/chapter8_modern_architectures.md